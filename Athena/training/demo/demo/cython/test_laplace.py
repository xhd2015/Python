...
functions implemented in pure Python, NumPy, and Cython.  To compare all the
options you need to have NumPy and Cython installed.

The cython modules are compiled using the setup.py script that is provided with
...
class LaplaceSolver(object):
    ...
    def cythonTimeStep(self, dt=0.0):
        """Takes a time step using a function written in Cython.  Use
        the given setup.py to build the extension using the command
        python setup.py build_ext --inplace.  You will need Cython
        installed to run this."""        
        g = self.grid
        err = pyx_lap.cythonTimeStep(g.u, g.dx, g.dy)
        return err
    ...
    def setTimeStepper(self, stepper='numeric'):        
        ...
        if stepper == 'slow':
            ...
            self.timeStep = self.cythonTimeStep
        else:
            self.timeStep = self.numericTimeStep            
...
def main(n=500, n_iter=100):
    ...
    for technique in ["Numeric", "cython"]:
	print "\t%s took %s seconds" % \
	      (technique, time_test(n, n, stepper="cython", n_iter=n_iter))
