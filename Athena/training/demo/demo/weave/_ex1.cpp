...
static PyObject* my_map(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occurred = 0;
    PyObject *py_local_dict = NULL;
    static const char *kwlist[] = {"func","seq","local_dict", NULL};
    PyObject *py_func, *py_seq;
    int func_used, seq_used;
    py_func = py_seq = NULL;
    func_used= seq_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OO|O:my_map",const_cast<char**>(kwlist),&py_func, &py_seq, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_func = py_func;
        ...
        func_used = 1;
        py_seq = py_seq;
        py::list seq = convert_to_list(py_seq,"seq");
        seq_used = 1;
        /*<function call here>*/     
...
{
    {"my_map",(PyCFunction)my_map , METH_VARARGS|METH_KEYWORDS},
    {NULL,      NULL}        /* Sentinel */
};

PyMODINIT_FUNC init_ex1(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("_ex1", compiled_methods);
}
